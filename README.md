...
### 2026-05-16 — Anthropic 协议原生透传 + reasoning 兼容层

- **Anthropic 客户端直通 MiMo 原生端点**：`/v1/messages` 不再走 IES 协议转换，handler 检测到 anthropic adapter 后直接把请求字节透传到 MiMo 的 `/anthropic/v1/messages`。`thinking` content block、`signature`、未来新增字段自动 round-trip。
- **入站补字段（兼容层核心）**：`patch_request_thinking` 扫描 assistant 历史，发现含 `tool_use` 块但缺 `thinking` 块的消息（一些 Anthropic 客户端会丢弃 thinking 以节省上下文），按 `tool_use.id` 集合查 reasoning_cache 自动补回，避免 MiMo 因缺 reasoning 返 400。
- **出站抓字段**：流式响应 tee 一份解析 Anthropic SSE 帧抓 `thinking_delta` + `tool_use` ids；非流式扫 response JSON 抓 `thinking` block + `tool_use` ids；都写进同一份 `reasoning_cache`，供下一轮入站补字段使用。
- **OpenAI Chat thinking 开关转发**：客户端 `extra_body={"thinking": {"type":"enabled"}}` 现在会被正确转发到上游，否则 MiMo 根本不进入思考模式。
- **流式即时缓存**：OpenAI 路径在拿到第一个 `tool_call.id` 时立即把已有 reasoning 写进缓存，避免流被客户端断开后缓存空空。
- **缓存观测**：`reasoning_cache` 加 `get_cache_stats()` 暴露 stores/hits/misses/expired 计数，方便事后判断兼容层是否真在做功。
- **ECS api-proxy 补 `anthropic-beta` header 转发**，支持 Anthropic 客户端使用 beta features。
...
- **兼容兜底**：Gateway 会按工具调用 ID 缓存上游 reasoning；当客户端/第三方项目丢弃该字段后，后续请求会自动补回。缓存未命中时不会伪造空 reasoning，避免把非思考工具调用错误升级成 400。
- **400 风暴防护**：上游 4xx 会记录指标但不再计入 backend health failure，避免请求格式问题导致健康后端轮流熔断。
