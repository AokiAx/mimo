...
- `MIMO_WS_PUBLIC_URL` — 本网关 `/ws` 隧道端点的**公网地址**（如 `wss://your-domain/ws`）。auto-deploy 会据此生成每账号的 bridge 回连地址（`?account=<账号>&token=...`）。**WS 部署必填**
...
- `MIMO_WS_TUNNEL_TOKEN` — 可选，保护 `/ws` Claw 节点接入通道；设置后 bridge URL 需带 `?token=...` 或 `Authorization: Bearer ...`
- `MIMO_WS_NODE_RESPONSE_TIMEOUT_S`、`MIMO_WS_STREAM_CHUNK_TIMEOUT_S`、`MIMO_WS_MAX_PENDING` — WS 隧道节点首包超时、流式 chunk 超时和最大挂起队列数
...
│   ├── auto_deploy.py      # 定时部署调度器（WS 隧道模式）
│   └── payload/
│       └── ws-bridge.py        # 注入 Claw 的出站 WS 桥接脚本（连回公网 /ws）
│
...
│   ├── ws_tunnel.py        # /ws 反向隧道：Claw bridge 节点接入后可作为 ws:// 后端
...
- **WS 反向隧道后端**：让 Claw 内的 `ws-bridge.py` 主动连回公网 `/ws`（mimi3 风格）。面板新增后端时 `base_url` 填 `wss://公网域名/ws?account=该账号`、`account` 填同一账号名，Gateway 按 account 把请求路由到对应 bridge 节点。
...
核心流程（WS 隧道模式，无跳板机 / 无 SSH）：
1. 通知 Gateway 准备替换（drain 旧后端）。
2. 销毁旧 Claw（若有）。
3. 创建新 Claw；遇到容量 429 会在预算内抖动重试。
4. 等待 Claw `AVAILABLE`，再发送模板恢复/重启测试。
5. 把 `ws-bridge.py`（已注入本账号的 `?account=<账号>&token=...` 回连地址）作为代码块发给 Claw，要求它安装依赖并用 `nohup` 后台运行。bridge 随即**主动连回**公网 `/ws`。
6. 轮询 `/ws` 隧道，确认该账号的 bridge 节点已接入；随后交给 Gateway 重载 + 热身，由热身探测做端到端模型链路验证。

每账号一个 bridge 节点，按 `account` 路由；缺少 `MIMO_WS_PUBLIC_URL`、payload 文件缺失、Claw 安全拒绝等问题会失败并落 incident 日志。

## WS 隧道模式（mimi3 风格）

1. 公网启动本项目并设置 `MIMO_WS_PUBLIC_URL=wss://your-domain/ws`，确保 `/ws` 可被 Claw 访问；如设置了 `MIMO_WS_TUNNEL_TOKEN`，bridge 回连地址会自动追加 `?token=你的token`。
2. auto-deploy 会自动把 `ws-bridge.py` 注入 Claw 运行。**手动**运行时（调试用）：

```bash
export MIMO_WS_TUNNEL_URL='wss://your-domain/ws?account=该账号&token=你的token'
# MIMO_API_KEY / MIMO_API_ENDPOINT 缺省时，bridge 会从 openclaw-gateway 进程环境(/proc)自动探测
python3 ws-bridge.py
```

3. 管理面板添加后端：`base_url` 填 `wss://your-domain/ws?account=该账号`，`account` 字段填同一账号名，models 填该账号支持的 MiMo 模型。Gateway 按 `account` 把请求路由到对应的 bridge 节点；热身探测会通过 WebSocket 隧道验证。
...
### 公开端点
- `GET /api/public/status` — 状态页数据 JSON（**需独立 `status_api_token`**：`Authorization: Bearer <token>` / `X-Status-Key` / `?key=`），返回脱敏聚合（总量、成功率、tokens、operational、在线后端数）
