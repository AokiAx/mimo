...
- SSH 部署本身的目标机 / 端口 / 管理私钥配置在 `data/ssh_targets.json`（见「自动部署流程」），不走环境变量。
...
│   ├── ssh_targets.json    # SSH 部署：目标机定义 + 每账号端口分配（参考 .example）
...
<details>
<summary>目标机一次性配置 + ssh_targets.json</summary>
...
**2. 面板填 `data/ssh_targets.json`**（参考 `data/ssh_targets.json.example`）：
