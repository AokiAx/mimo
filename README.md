...
- `DISABLE_CLAW_ACTIVITY=1` — 只关闭 Claw 存活/健康监控与 relay 循环；Gateway 后台测活仍会启动
...
- `GATEWAY_DRAIN_TIMEOUT_S`、`GATEWAY_DEPLOY_DRAIN_GRACE_S` — Gateway 后端 drain 和部署切换等待时间
...
│   ├── claw_activity.py    # Claw 存活/健康监控、账号池 relay 与风险隔离
...
- **模型能力预检**：图片等多模态输入仅 `mimo-v2.5` 原生支持（V2 系 2026-06-30 已下线）；其它模型在 Gateway 层直接 400。
...
- **号池模型**：可用 / 服役 / 冷却 / 风控 / ck死 / 停用（见 `claw/account_pool.py`）；`bannedStatus` 与 create 风控会进 risk 池。
- **账号接力（2h 开新号）**：免费 Claw ≈4h TTL；**每 2 小时**用新账号 create 一只 Claw，保证重叠；每只 Claw **最后 30 分钟** Gateway 置 `draining`（不接新请求）直到到期禁用。
- **多 active 后端**：允许同时有多只 Claw 接流量；路由优先选**最新** active；仅手动「激活」会独占并排空其它后端。
- **熔断**：连续失败 3 次后冷却 30s。`enabled` 开关可独立禁用后端。
- **主动 chat 测活**：默认每 30s 探测当前 active 后端的 `/v1/chat/completions`；连续失败触发短暂熔断，下一次探测成功后恢复。用户请求只写 metrics，不更新后端健康评级，避免客户端请求格式、取消流或侧边异常把健康后端降级。
...
`claw/auto_deploy.py` 负责单次部署执行，`claw/claw_activity.py` 负责存活/健康监控、账号池 relay 和风险隔离。运行历史写入 `data/deploy_history/`，详细日志写入 `data/deploy_logs/`。Claw 自身的反向隧道/代理由部署时装到 claw 本地的 `tunnel-keepalive.sh` cron（每 2 分钟）保活，面板侧不再通过 WS 对话去维护。
...
当前 free-tier：单个 Claw ≈4h TTL，每账号每天约 1 次 create。activity loop **每 2h 开一只新 Claw**（多号接力）；Gateway 在寿命最后 30 分钟自动 draining。全天覆盖大约需要 12 个可用自动号（`MIMO_CK_POOL_TARGET` 默认 12）。
...
### 公开端点
- `GET /api/public/status` — 状态页数据 JSON（**需独立 `status_api_token`**：`Authorization: Bearer <token>` / `X-Status-Key` / `?key=`），返回脱敏聚合（不含后端身份）。各区间由响应里的 `windows` 字段标注：
  - 累计：`total_requests` / 成功率 / tokens / 延迟·TTFT 分位（p50/p95/p99/avg）
  - 近 24h：`window_24h`（请求/成功/错误/成功率/tokens/延迟/TTFT/流式·非流式计数）、`status_codes` 分布、Top `models` / `routes`
  - 趋势：`hourly`（48h 分时，带可用性状态）、`daily`（30 天日次，北京日界）
  - 可用率：`uptime`（24h / 48h 健康占比 %；degraded 半权重，无流量为 null）
  - 实时：`status`（operational/degraded）、`backends_online`、`generated_at`（生成时刻 epoch）
...
# 注册新小米账号（额外依赖 curl_cffi pycryptodome；全自动再加 requests）
python register_mimo.py --email X --password Y          # 半自动（邮箱码手输）
python register_mimo.py --auto --captcha-key $KEY       # 全自动：mail.tm + 2Captcha
python register_mimo.py --auto --count 3 --enable-deploy
# IMAP catch-all：
#   --mailbox imap --imap-address 'tag*@your.domain' --imap-host ... --imap-user ... --imap-password ...
```

全自动说明见脚本头注释：图形码用 `CAPTCHA_API_KEY`（2Captcha）；邮箱码默认 `mail.tm`（域名可能被小米拒，稳妥用 IMAP）。成功写入 `accounts/<email>.json`，`--enable-deploy` 会打开 auto_deploy。
