...
### （可选）配置 SSH 自动部署
要用「定时把 Claw 变成 API 转发节点」，按下文「自动部署流程（SSH 反向隧道 · 方案 B）」配置 `data/config.json` 的 `ssh_targets` 节 + 目标机一次性 `setup-target.sh`。
...
│   ├── auto_deploy.py      # 定时部署调度器（SSH 反向隧道 · 方案 B）
...
│       ├── setup-target.sh         # 建 nologin tunnel 用户 + 安装授权器 + 写面板公钥
...
│   ├── config.json         # 统一配置：backends / model_groups / auto_deploy /
│   │                       #   panel_acl / probe_nodes / ssh_targets / pin 分节
...
- **模型映射**：`data/config.json` 的 `model_groups` 节管理对外模型名到 MiMo 原生模型的映射；`/v1/models` 会优先返回映射后的 exposed model。
...
`claw/auto_deploy.py` 按账号调度部署，运行历史写入 `data/deploy_history/`，详细日志写入 `data/deploy_logs/`。
...
8. 轮询目标机 `http://<上游>:<端口>/health` 就绪后，登记/刷新该账号后端，交给 Gateway 重载 + 热身。
...
**1. 目标机（你的中转机器）跑一次**（创建 nologin `tunnel` 用户 + 安装强制命令授权器 + 写入面板管理公钥）：
...
**2. 面板填 `data/config.json` 的 `ssh_targets` 节**（参考 `data/config.json.example`）：
