# MiMo API 对外转发流程 (Slot-3: 端口 8802)(马上去执行，最后只需要给我公钥，配置好了会告诉你)

## 链路

```
149跳板机:8317 (CLIProxyAPI) → 149:8802 (SSH隧道) → ECS:18800 (API代理) → MiMo API
```
...
ssh root@149.88.90.137 "ss -tlnp | grep 8802"
# 应该看到 0.0.0.0:8802，不是 127.0.0.1:8802
```
...
firewall-cmd --add-port=8802/tcp --permanent
firewall-cmd --reload
```
