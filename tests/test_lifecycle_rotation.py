...
def test_router_excludes_inactive_and_draining_backends():
    active = _backend("active")
    inactive = _backend("inactive", lifecycle="inactive")
    draining = _backend("draining", lifecycle="draining")
    router = Router(BackendRegistry([inactive, draining, active]))

    chosen, decision = router.choose(request_id="r1", model="mimo-v2.5-pro")

    assert chosen.backend_id == "active"
    assert decision.excluded["inactive"] == "lifecycle=inactive"
    assert decision.excluded["draining"] == "lifecycle=draining"


def test_router_raises_when_only_inactive_backend_exists():
    router = Router(BackendRegistry([_backend("inactive", lifecycle="inactive")]))

    with pytest.raises(BackendUnavailableError):
        router.choose(request_id="r1", model="mimo-v2.5-pro")


def test_router_excludes_inactive_backend():
    active = _backend("active")
    inactive = _backend("inactive", lifecycle="inactive")
    router = Router(BackendRegistry([inactive, active]))
...
def test_activate_backend_drains_existing_active_peers(monkeypatch):
    ...
    new = _backend("new", lifecycle="inactive")
...
def test_toggle_backend_enable_activates_disabled_backend(monkeypatch):
    old = _backend("old")
    disabled = _backend("disabled", lifecycle="disabled")
    disabled.enabled = False
    reg = BackendRegistry([old, disabled])
    monkeypatch.setattr(runtime, "_registry", reg)
    monkeypatch.setattr(runtime, "_persist_backend_runtime_state", lambda _backend: None)
    monkeypatch.setattr(backend_store, "update_backend", lambda *_args, **_kwargs: {})

    result = runtime.toggle_backend("disabled")
...
def test_probe_one_uses_chat_completion_not_models(monkeypatch):
    ...
    ok, reason = asyncio.run(
        runtime._run_one_probe_check(
            backend,
            "probe",
            runtime._probe_non_stream_body(backend),
        )
    )
    ...
    assert fake.json_bodies[0]["messages"][0]["content"] == runtime._PROBE_PROMPT
...
def test_probe_model_falls_back_when_flash_unavailable():
    backend = _backend("candidate")
    backend.models = ["mimo-v2.5-pro"]

    assert runtime._probe_model(backend) == "mimo-v2.5-pro"
...
def test_persist_backend_runtime_state_logs_failures(monkeypatch, caplog):
    backend = _backend("candidate", lifecycle="inactive")
...
def test_retire_other_backends_leaves_inactive_backends_alone(monkeypatch):
    chosen = _backend("chosen")
    inactive = _backend("inactive", lifecycle="inactive")
    reg = BackendRegistry([chosen, inactive])
    monkeypatch.setattr(runtime, "_registry", reg)
    monkeypatch.setattr(runtime, "_persist_backend_runtime_state", lambda _backend: None)
...
def test_abort_account_deploy_restores_restorable_draining_backend(monkeypatch):
    old = _backend("old", lifecycle="draining")
    old.account_id = "alice"
    peer = _backend("peer")
    peer.account_id = "bob"
    ...
    result = runtime.abort_account_deploy("alice", restore=True, error="cancelled")
...
def test_abort_account_deploy_marks_unrestorable_backend_failed(monkeypatch):
    old = _backend("old", lifecycle="draining")
    old.account_id = "alice"
    reg = BackendRegistry([old])
    monkeypatch.setattr(runtime, "_registry", reg)
    monkeypatch.setattr(runtime, "_persist_backend_runtime_state", lambda _backend: None)

    result = runtime.abort_account_deploy("alice", restore=False, error="cancelled")
...
def test_status_label_reflects_states():
    ...
    assert b.status_label() == "inactive"
